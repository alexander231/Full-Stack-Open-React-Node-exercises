...
describe('most blogs', () => {
    ...
    test('when list has more blogs, return the author with most blogs', () => {
        const result = listHelper.mostBlogs(listWithMoreBlogs)
        expect(result).toStrictEqual({author: 'Edsger W. Dijkstra', blogs: 2})
    })
    test('when list has even more blogs, return the autor with most', () => {
      const result = listHelper.mostBlogs(listWithEvenMoreBlogs)
